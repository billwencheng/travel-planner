...
async def validate_preferences_tool(
    ...
    if part and getattr(part, "text", None):
        try:
            ...
            for index, hotel in enumerate(hotels):
                if index == 0:
                    cost += (
                        float(hotel.get("price_per_night", 0)) * 5
                    )  # Assume 5 nights based on generate_vibe_diff_tool
    ...
    return ValidationReport(
        isAligned=is_aligned, approvedDataURI=searchDataURI if is_aligned else ""
    )


async def generate_vibe_diff_tool(
    approvedDataURI: str, tool_context: ToolContext
) -> list[dict]:
    ...
    if not summary_sentences:
        ...
        cost = 850.0
        deep_links = [
            "https://booking.com/example",
            "https://flights.google.com/example",
        ]
