...
async def test_full_search_flow():
    ...
    msg = types.Content(
        ...
        parts=[
            types.Part.from_text(
                text="I want to travel from NYC to MIA from 2026-10-10 to 2026-10-15. 2 travelers."
            )
        ],
    )

    session = get_session_service().create_session_sync(
    ...
    assert (
        "MIA" in final_output
        or "card" in final_output.lower()
        or "cost" in final_output.lower()
        or "vibe" in final_output.lower()
        or "smooth" in final_output.lower()
    ), "Final output should reflect the trip details, A2UI cards, or vibe."
